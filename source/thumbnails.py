...
from PyQt6.QtCore import pyqtSlot, pyqtSignal, QObject, QMutex, QRunnable, QThreadPool, QUrl, QByteArray, QSize
from PyQt6.QtQuick import QQuickImageProvider, QQuickAsyncImageProvider, QQuickImageResponse, QQuickTextureFactory
...
class ThumbnailResponseRunnablepyqtSignals(QObject):
...
class ThumbnailResponseRunnable(QRunnable):
    def __init__(self, file, size, quality):
        ...
        self.file = file
        self.signals = ThumbnailResponseRunnablepyqtSignals()
        self.image = None
...
class SyncThumbnailProvider(QQuickImageProvider):
    def __init__(self, size, quality):
        super(SyncThumbnailProvider, self).__init__(QQuickImageProvider.ImageType.Image)
        self.size = size
        self.quality = quality
