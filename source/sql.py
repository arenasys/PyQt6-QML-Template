...
from PyQt6.QtCore import pyqtProperty, pyqtSlot, pyqtSignal, Qt, QObject, QThread, QAbstractListModel, QByteArray, QModelIndex, QTimer, QVariant
...
class Connection(QObject):
    ...
    def connect(self):
        ...
        db.open()
        db.driver().notification.connect(Database.instance.onNotification)
...
class Sql(QAbstractListModel):
    ...
    def updateFieldNames(self, record):
        self.fieldNames = {}
        self.fieldNames[Qt.ItemDataRole.UserRole] = QByteArray(("data").encode("utf-8"))
        for i in range(len(record)):
            self.fieldNames[Qt.ItemDataRole.UserRole + i + 1] = QByteArray(("sql_" + record.fieldName(i)).encode("utf-8"))
